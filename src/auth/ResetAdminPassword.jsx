...
import { RESET_ADMIN_PASSWORD } from '../graphql/mutations';
...
const ResetAdminPassword = () => {
  ...
  const [resetAdminPassword, { data, loading, error }] = useMutation(RESET_ADMIN_PASSWORD);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await resetAdminPassword({ variables: { token, newPassword } });
  ...
  return (
   ...
   <div className='relative'>
      ...
      <button type="submit" disabled={loading} className="w-full p-2 bg-red-500 text-white rounded-md hover:bg-red-600 transition-colors duration-300 disabled:opacity-50">
        {loading ? 'Resetting...' : 'Reset Password'}
      </button>
      {error && <p className='text-red-500 text-sm'>Error: {error.message}</p>}
      {data && <p className='text-green-600 text-sm'>{data.resetAdminPassword.message}</p>}
    </form>
    </div>
  );
};
