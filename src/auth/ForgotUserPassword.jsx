...
const ForgotUserPassword = () => {
    ...
    const [forgotUserPassword, { data, loading, error }] = useMutation(FORGOT_USER_PASSWORD);

    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await forgotUserPassword({ variables: { email } });
    ...
    return (
        ...
        >
          ...
          <div className="space-y-2">
            ...
            {data && <p className='text-green-500 text-sm'>{data.forgotUserPassword.message}</p>}
        </form>
    </div>
    );
};
